...
import { Users, Plus, Eye, Pencil, Trash2, UserPlus } from 'lucide-react';
...
export function PatientsPage() {
  ...
  const load = useCallback(() => {
    setLoading(true);
    setError(false);
    patientApi.list().then(setPatients).catch(() => setError(true)).finally(() => setLoading(false));
  }, []);

  useEffect(() => { load(); }, [load]);

  const filtered = patients
    .filter((p) => p.name.toLowerCase().includes(search.toLowerCase()) || p.patientCode.toLowerCase().includes(search.toLowerCase()) || p.email.toLowerCase().includes(search.toLowerCase()))
    .sort((a, b) => {
      const dir = sortDir === 'asc' ? 1 : -1;
      return (a[sortBy as keyof Patient] as string).localeCompare(b[sortBy as keyof Patient] as string) * dir;
    });
  ...
  const handleSort = (key: string) => {
    if (sortBy === key) { setSortDir(sortDir === 'asc' ? 'desc' : 'asc'); } else { setSortBy(key); setSortDir('asc'); }
  };
  ...
  const handleSave = async () => {
    ...
    else { const newPatient = { ...form, patientCode: form.patientCode || `PT-2024-${String(patients.length + 1).padStart(3, '0')}` }; await patientApi.create(newPatient); }
    setModalOpen(false);
    load();
  };

  const handleDelete = async () => {
    if (deleteTarget) { await patientApi.remove(deleteTarget.id); load(); }
  };
  ...
  const columns: Column<Patient>[] = [
    ...
    { key: 'actions', header: 'Actions', align: 'right', render: (p) => (
      <div className="flex items-center justify-end gap-1">
        ...
        <button onClick={(e) => { e.stopPropagation(); setDeleteTarget(p); }} className="rounded-lg p-1.5 text-gray-400 transition-colors hover:bg-gray-100 hover:text-error-600"><Trash2 className="h-4 w-4" /></button>
      </div>
    )},
  ];
  ...
  return (
    <div className="space-y-6">
      <div className="grid gap-4 sm:grid-cols-3">
        <StatCard label="Total Patients" value={patients.length} icon={Users} color="primary" />
        <StatCard label="Active" value={activeCount} icon={Users} color="success" />
        <StatCard label="Admitted" value={admittedCount} icon={Users} color="warning" />
      </div>
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title={editing ? 'Edit Patient' : 'Add New Patient'} description={editing ? `Updating ${editing.name}` : 'Enter patient details below'} size="lg"
        footer={<><button className="btn-secondary" onClick={() => setModalOpen(false)}>Cancel</button><button className="btn-primary" onClick={handleSave}>{editing ? 'Save Changes' : 'Add Patient'}</button></>}
      >
        <div className="grid gap-4 sm:grid-cols-2">
          <Field label="Full Name"><input className="input-field" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} placeholder="John Doe" /></Field>
          <Field label="Email"><input className="input-field" type="email" value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} placeholder="john@email.com" /></Field>
          <Field label="Phone"><input className="input-field" value={form.phone} onChange={(e) => setForm({ ...form, phone: e.target.value })} placeholder="+1 555 000 0000" /></Field>
          ...
          <Field label="Emergency Contact"><input className="input-field" value={form.emergencyContact} onChange={(e) => setForm({ ...form, emergencyContact: e.target.value })} placeholder="+1 555 000 0000" /></Field>
          ...
          <div className="sm:col-span-2"><Field label="Address"><input className="input-field" value={form.address} onChange={(e) => setForm({ ...form, address: e.target.value })} placeholder="123 Main St, City" /></Field></div>
        </div>
      </Modal>
      ...
      <ConfirmDialog open={!!deleteTarget} onClose={() => setDeleteTarget(null)} onConfirm={handleDelete} title="Delete Patient" message={`Are you sure you want to delete ${deleteTarget?.name}? This action cannot be undone.`} confirmLabel="Delete" danger />
    </div>
  );
}
